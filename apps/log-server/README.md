...
The reference destination for Vinaya log events: a Cloudflare Worker and one SQLite-backed Durable Object per repository. It receives the NDJSON a Vinaya installation delivers to its `logs.url` destination, stores every event exactly once, and pages it back from any position.
...
| `src/repo-log.ts` | `RepoLog`, the Durable Object: ingest, deduplication, storage, paging, stats |
...
`INGEST_TOKEN` is write-only and accepted on the ingest route alone; `READ_TOKEN` is read-only and accepted on the read and stats routes alone. Both are Worker secrets and never live in this repository.
