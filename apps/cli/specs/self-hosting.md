...
One publication note: `aeg-root/` is in this package's `files` array and reaches every adopter in the tarball, while `specs/` is not published at all. `README.md` carries a short form of this claim, and together with `enforcement.md` it is the account an adopter actually gets.
