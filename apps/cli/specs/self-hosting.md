...
`contents: write` in the job's `permissions:` block is still spent by `GITHUB_TOKEN`, not by the fine-grained token — `actions/checkout` persists it, and that credential is what the action's `git push` of the release branch uses. Only the API calls read the step's `env`.
