...
**Which outbox is read vs. where it posts can now differ (`task-log-v1` 8, Issue #626, O1).** `FlushOptions.outboxTask` decouples the two: absent (every explicit `vinaya log flush --issue/--pr` call, and `dispatch.ts`'s own trailing flush), the outbox read/truncated is still `target`'s own resolved issue number, exactly the original, only behavior. The developer-review loop's round-end flush is the one caller that sets it — reading task `<n>`'s own outbox (`outboxTask: n`) while posting to a SEPARATELY configured destination (`vinaya.config.json`'s `logPublish`, `apps/cli/specs/loop.md` § The round-end flush) — because that flush's whole point is to stop posting new comments onto the task's own Issue by default, the surface `fetchFrozenBrief` must read to dispatch it.
