...
**Which outbox is read vs. where it posts can now differ (`task-log-v1` 8, Issue #626, O1).** `FlushOptions.outboxTask` decouples the two: absent (every explicit `vinaya log flush --issue/--pr` call), the outbox read/truncated is still `target`'s own resolved issue number, exactly the original, only behavior. The developer-review loop's round-end flush and `commands/dispatch.ts`'s own trailing flush (Issue #636, O4 — see below) both set it — reading the dispatched task's own outbox (`outboxTask: n`) while posting to a SEPARATELY configured destination (`vinaya.config.json`'s `logPublish`, `apps/cli/specs/loop.md` § The round-end flush) — because both flushes' whole point is to stop posting new comments onto the task's own Issue by default, the surface `fetchFrozenBrief` must read to dispatch it.

**Published volume is bounded per call (O2).** `FlushOptions.maxChunksPerFlush` (default `DEFAULT_MAX_CHUNKS_PER_FLUSH`, `apps/cli/src/lib/config.ts`) caps how many chunks — comments — ONE `flushOutbox` call posts. A chunk beyond the bound is simply never attempted this call: not posted, not truncated, not counted as a duplicate on the next attempt — it stays exactly where it was in the outbox for a LATER call to pick up, in the same FIFO order (`planFlush`'s own chunk order is never reshuffled). `LogFlushOutcome.deferredChunkCount` names how many chunks a call left behind; every caller surfaces a non-zero value visibly (the round-end flush and `vinaya dispatch`'s trailing flush to stderr, `vinaya log flush` to stdout) — partial coverage this round, per the Log contract's own loss/coverage-visibility requirement, never silently reported as a complete flush. The one-shot `vinaya log flush` command is bounded too, by the same default; its own `skipRemotelyAccepted: true` idempotency makes re-running it to drain a larger backlog safe.

The developer review loop's round-end flush (`apps/cli/specs/loop.md`) calls `flushOutbox` the same way, in-process — never `vinaya log flush` as a subprocess of its own CLI entry, which `apps/cli/specs/surface.md`'s "commands never call commands" rule forbids as much through a child process as through a direct call. `commands/dispatch.ts`'s own trailing flush calls the identical function directly too, for the identical reason. Both callers catch a thrown `LogFlushError` and treat it as non-fatal — a flush failure never undoes the effect that produced the lines still waiting to be flushed.

**`vinaya dispatch`'s trailing flush publishes to `logPublish`, never to `--task`/`--pr` directly (Issue #636, O4).** Before this fix, `commands/dispatch.ts`'s own trailing flush built its destination straight from the CLI's `--task`/`--pr` flags — posting straight onto the dispatched task's own Issue/PR every time, regardless of what (or whether) `vinaya.config.json`'s `logPublish` configured, and with no way to opt into "publish nowhere" the way the round-end flush already had. It now resolves its destination the same way the round-end flush does — `resolveRoundEndFlushTarget`/`describeSkippedRoundEndFlush` (`apps/cli/src/lib/config.ts`), reading the dispatched task's own outbox (`outboxTask`) while posting to a SEPARATE configured destination — so both flush call sites share one rule: no `logPublish` configured means publish nowhere, reported on stderr rather than silently skipped, and a configured target equal to the task's own Issue is refused the same way. `--task`/`--pr` still select which task's local outbox to drain; they are never the publish destination.
...
### The webhook destination — a plain HTTP alternative to a GitHub comment

`logPublish.webhookUrl` (optionally with `headers`) is a third destination alongside `logPublish.issue`/`pr`, mutually exclusive with both: instead of a GitHub comment, the flush POSTs the outbox to any HTTP endpoint — no `gh` auth or GitHub account needed on the receiving end. `flushOutboxToWebhook` (`apps/cli/src/lib/log-webhook-flush.ts`) is the implementation; `vinaya log flush --issue/--pr`, the round-end auto-flush (`dev-review-loop.ts`'s `defaultFlushOutbox`), and `vinaya dispatch`'s own trailing flush all branch to it when the resolved target carries a `webhookUrl`.

<!-- AEG:CLAIM: apps/cli/src/lib/log-webhook-flush.ts contains:export async function flushOutboxToWebhook( -->

It deliberately reuses less than the GitHub path: the same `classifyStoredLine` validation/redaction (fail-closed on any corrupt or unknown-version line, exactly as `flushOutbox` runs before a GitHub post) and the same tail-preserving truncate-only-on-confirmed-success rule, but none of `flushOutbox`'s comment-size chunking (`planFlush`/`FORGE_COMMENT_MAX_CHARS`) or marker-based idempotent-retry read — a generic endpoint has no per-comment size limit to chunk around and no comment history to read back and dedupe against. One call POSTs the whole outbox as a single ndjson body (`Content-Type: application/x-ndjson`, plus any configured `headers`), bounded at `WEBHOOK_FETCH_TIMEOUT_MS` (30s — round-2 security review, MEDIUM: an unresponsive endpoint would otherwise hang the call, and with it the round-end auto-flush and the whole dev-review-loop, indefinitely); a body over `MAX_WEBHOOK_BODY_BYTES` (5 MiB) refuses rather than silently splitting, so an adopter who outgrows it flushes more often instead of relying on multi-POST delivery this path never guarantees. A non-2xx response, a network failure (including a timeout), or a corrupt line all refuse with the outbox left untouched, safe to retry; a 2xx truncates it.

**The round-end auto-flush requires the trust anchor's own agreement before honoring a `webhookUrl` (round-2 security review, HIGH).** `resolveLogPublishTarget` reads `vinaya.config.json` off the working tree being reviewed — fine for `issue`/`pr`, both forge-internal, but a `webhookUrl` is an arbitrary outbound HTTP destination, so an UNATTENDED caller honoring one straight from a PR's own diff would let that PR redirect task telemetry to an attacker-chosen host on the very next round. `defaultFlushOutbox` therefore additionally checks `resolveTrustAnchorWebhookTarget` (`apps/cli/src/lib/config.ts`) before ever POSTing: only a `webhookUrl` — and its `headers` — already present on the repository's DEFAULT BRANCH (`loadTrustAnchorConfig`, the same server-side read `principals`/`tokens.collect` use) is honored; a working-tree `webhookUrl` that doesn't match is refused, visibly, on stderr, same as an unconfigured one. `vinaya log flush` and `vinaya dispatch`'s trailing flush are not gated this way — a human is choosing to run either, the same trust level as running any other `vinaya.config.json`-declared command.

Because a webhook target has no comment history, `journal-history.ts`'s `fetchFlushedLoopEvents` — which replays a task's already-flushed `dev_review_loop` events from the GitHub comments `flushOutbox` posted — treats a webhook target the same as no `logPublish` configured at all: it skips the forge read and relies on the local, unflushed outbox alone.
