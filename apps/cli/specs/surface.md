...
`killGraceMs` (task-run-v1 task 20, O2) is a new optional field on two already-exported surfaces — `VinayaConfigSchema`'s `dispatch` object (`apps/cli/src/lib/config.ts`) and `RunOptions` (`apps/cli/src/checks/runner.ts`) — per this file's own rule (`## The rule`), a field on an already-exported type/const needs no new row. It overrides the SIGTERM-to-SIGKILL grace window `dispatchRole` and `runChecks` each already enforced at a hardcoded constant, so a test proving the escalation itself happens can configure a small, real, non-zero window instead of paying the production-sized default. Neither function's own exported surface or exemption status changes.
