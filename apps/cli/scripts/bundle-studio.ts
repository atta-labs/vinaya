 ...
 * its own `next.config.ts`) from inside a monorepo that carries Studio's
 * source — the only place workspace resolution of `@atta/*` packages and the
 * `@atta/ui` codegen step actually work — then copies the resulting
 * `.next/standalone` + `.next/static` + `public/` into
 * `apps/cli/studio-standalone/`, a build-time artifact (gitignored, same
 * convention `bundle-doctrine.ts` already uses for `aeg-root/`) that
 * `npm pack`/`npm publish` would ship as a normal top-level package
 * directory.
 *
 * THIS repository does not contain that source: the CLI was extracted here
 * while `apps/vinaya/web` stayed behind in the attalabs monorepo, so the
 * guard below refuses on every run from this tree. The script is recovered
 * anyway because everything downstream of the guard is the working
 * pack-shape knowledge (`_node_modules` rename, tracer-recursion clean,
 * baked-facts deletion) the Studio-packaging task (#43) needs when it makes
 * a source or fetched bundle available — until then it is deliberately NOT
 * wired into `prepack` or the `files` allowlist, so publishing never runs
 * it and the published artifact is unchanged by its presence.
 *
...
const webRoot = join(repoRoot, 'apps', 'vinaya', 'web')
const targetRoot = join(pkgRoot, 'studio-standalone')

if (!existsSync(webRoot)) {
  console.error(
    `bundle-studio: source ${webRoot} does not exist — Studio's source (apps/vinaya/web) lives in the attalabs monorepo, not this repository. Making a Studio source or bundle available here is the Studio-packaging task (#43); this script becomes runnable once one exists at that path.`
  )
  process.exit(1)
}
