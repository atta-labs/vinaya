...
- `pr-body-frozen` (ring 1, `requiresOpenPr`) is a worked example of a check that needs more than `PR_BODY`: it fetches the PR's own body and comments via `gh` to find the `aeg:body-hash` marker `vinaya pr create` posts at open, recomputes the same hash from the live body, and fails on mismatch. A missing marker (posted by a non-allowlisted author, or absent entirely) is grandfathered — an `info`, never a `fail` — only for a PR opened before the rollout cutoff (`FROZEN_BODY_SINCE_PR`); a PR opened at or after the cutoff with no marker `fail`s instead, so deleting the marker comment cannot buy back the grandfathered case.
