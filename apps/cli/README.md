...
- `pr-body-frozen` (ring 1, `requiresOpenPr`) is a worked example of a check that needs more than `PR_BODY`: it fetches the PR's own comments via `gh` to find the `aeg:body-hash` marker `vinaya pr create` posts at open, recomputes the same hash from the live body, and fails on mismatch — grandfathering (an `info`, never a `fail`) any PR whose marker is absent or posted by a non-allowlisted author.
