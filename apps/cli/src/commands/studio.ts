...
export function resolveStudioTarget(cwd: string, moduleUrl: string = import.meta.url): StudioTarget {
  ...
  for (;;) {
    ...
    // `vinaya studio` runs from anywhere beneath it (a security-review
    // finding). Checked AFTER the webDir probe so a monorepo root that
...
function spawnDev(webDir: string, args: string[]): Promise<number> {
  return new Promise((resolve) => {
    ...
    // deliberately, not by omission (a security-review finding). The
    ...
    // work whose published shape DOES get the loopback default below.
 ...
 *  to — not `0.0.0.0` (a security-review finding): detection is
 ...
 *  tranche/task data with no auth (a security-review finding). Loopback-only
