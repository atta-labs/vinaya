...
//
// The type vocabulary is `@attalabs/aeg-core`'s `COMMIT_TYPE_STYLE` — the
// same regex `checkForgeTitle` enforces on PR/Issue titles. One list, two
// call sites; never a second copy here (Issue #63).
//
...
export function commitMsgCommand(args: string[]): void {
  ...
  process.stderr.write(
    `vinaya commit-msg: "${firstLine}" doesn't match this repo's commit convention.\n` +
      ...
      'Build, Chore, Docs, Feat, Fix, Perf, Plan, Refactor, Revert, Style, Test — start-case, ' +
      'a colon, a space, then the description.\n' +
