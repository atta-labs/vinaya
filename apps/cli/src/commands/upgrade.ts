...
/** A manifest-recorded file that is retired outright — an agent-skill file whose role was removed, or a managed artifact (e.g. the collector workflow) nothing generates any more — removed on apply, dropped from the manifest. */
...
export function planUpgrade(
  ...
  const staleFiles: StaleFile[] = staleManagedPaths.map((path) => ({
    path,
...
export function renderUpgradeDiff(plan: UpgradePlan): string {
  ...
  if (plan.staleFiles.length > 0) {
    lines.push('── Retired managed files ─────────────────────────────')
    for (const f of plan.staleFiles) {
      lines.push(
        f.present
          ? `  - remove ${f.path} (no longer generated by this version — retired)`
