...
${vinayaSetupSteps(selfHost)}${adopterSetupStep(ciSetup)}      - name: Run checks
        env:
          ...
          # PR_AUTHOR is a trust signal, not a display value — body-bare-digits'
          # Changesets-release exemption keys on BOTH this and BRANCH matching,
          # never BRANCH alone: a branch name is attacker-controlled (anyone
          # can push a branch literally called changeset-release/main), so
          # gating only on it would let a forged PR from that branch name
          # bypass the check for arbitrary content. github.event.pull_request's
          # own author field is set by GitHub itself from the real opener's
          # identity, not from anything the PR's branch or body can spoof.
