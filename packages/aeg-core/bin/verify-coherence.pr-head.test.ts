...
import { loadTrancheFiles } from './verify-coherence'
 ...
 * tranche row) has no forge equivalent to derive from yet.
 *
 ...
 * new, synthetic tranche file with a task referencing Issue `#999999` —
 * chosen specifically because it cannot possibly exist in the real forge.
 * If `loadTrancheFiles` read this task from anywhere OTHER than the
...
function buildSyntheticPrHeadCommit(slug: string, taskId: string, taskTitle: string): string {
  ...
  const content = [
    ...
    'Lifecycle: active',
    '',
    'Goal: synthetic fixture for Part 4 (PR-head-SHA path test), not a real tranche.',
    '',
    '## Tasks (topology)',
    '',
...
describe('PR-head-SHA path (plan-PR scoping) — unchanged by the forge cutover', () => {
  it("reads a synthetic tranche file's task from the constructed commit, not the forge or origin/main", async () => {
    ...
    // paying for a full forge-derive pass over every real tranche, which
    // this test has no interest in.
    const files = await loadTrancheFiles(
      {
        prHeadSha: commitSha,
    ...
    expect(file?.tranche.tasks).toHaveLength(1)
    expect(file?.tranche.tasks[0]).toMatchObject({
      id: '1',
