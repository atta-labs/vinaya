 ...
 * (Archivist's D-056 close-out), where GitHub's `closer` field never
 * populates even though a real merged PR did the work.
 *
 * The brief's original proposal (search merged PRs for a `Closes #N` link in
 * the body) was tested against the live fixtures and found insufficient: the
 * real closing PRs for #167/#168/#169 (#148/#150/#156) never carried a
 * `Closes #N` link at all — that omission is exactly why the Archivist had
 * to close them manually in the first place (each Issue's own archival
 * comment records "PR body did not carry a 'Closes #N' link"). A body-regex
