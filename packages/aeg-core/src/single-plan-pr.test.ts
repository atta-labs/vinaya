...
describe('trancheSlugFromTopologyPath', () => {
  it('parses the slug from an active tranche topology file', () => {
    expect(trancheSlugFromTopologyPath('aeg-root/tranches/aeg-governance-hardening.md')).toBe(
      'aeg-governance-hardening'
    )
  })

  it('returns null for a completed/ (archived) topology file', () => {
    expect(trancheSlugFromTopologyPath('aeg-root/tranches/completed/vada-production-v1.md')).toBeNull()
  })
  ...
  it('returns null for a .tokens.md ledger file', () => {
    expect(trancheSlugFromTopologyPath('aeg-root/tranches/aeg-governance-hardening.tokens.md')).toBeNull()
  })
...
describe('checkSinglePlanPr — single-plan-PR guard (task 19 / #336)', () => {
  it('passes trivially for an ordinary task-branch PR (no topology file touched)', () => {
    ...
    const otherOpenPrs = [{ number: 100, files: ['aeg-root/tranches/aeg-governance-hardening.md'] }]
  ...
  it('passes when two plan-branch diffs touch DIFFERENT tranches topology files', () => {
    const branchFiles = ['aeg-root/tranches/aeg-governance-hardening.md']
    const otherOpenPrs = [{ number: 200, files: ['aeg-root/tranches/aeg-forge-state-v1.md'] }]
    expect(checkSinglePlanPr(branchFiles, otherOpenPrs)).toEqual({ ok: true })
  })

  it('refuses when two plan-branch diffs touch the SAME tranche topology file, naming the other PR', () => {
    const branchFiles = ['aeg-root/tranches/aeg-governance-hardening.md']
    const otherOpenPrs = [{ number: 352, files: ['aeg-root/tranches/aeg-governance-hardening.md'] }]
  ...
  it('ignores an archived (completed/) topology file touched by another PR', () => {
    const branchFiles = ['aeg-root/tranches/aeg-governance-hardening.md']
    const otherOpenPrs = [{ number: 300, files: ['aeg-root/tranches/completed/aeg-governance-hardening.md'] }]
    expect(checkSinglePlanPr(branchFiles, otherOpenPrs)).toEqual({ ok: true })
  })
})
...
describe('touchesAnyTopology', () => {
  it('is true when at least one file is an active topology file', () => {
    expect(touchesAnyTopology(['README.md', 'aeg-root/tranches/aeg-governance-hardening.md'])).toBe(true)
  })
