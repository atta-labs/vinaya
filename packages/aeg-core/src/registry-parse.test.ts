...
`
  ...
  it('parses the real enforcement.md — 43 rows (34 plus the six standalone-shim rows G2 demanded, plus the retired-vocabulary row task 7 added, plus the G6 row task 8 added, plus the doctrine-portability row task 234 added), only non-deterministic rows carry an empty implementation', () => {
    ...
    expect(ring0Count + ring1Count + ring2Count).toBe(43)
