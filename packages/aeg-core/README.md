...
- **The Vinaya Log's typed policy layer** (`src/log/`) — `LogEventSchema` (a zod union over the `dispatch`, `dev_review_loop`, and `forge_write` families), the pure `buildHeader(input)` envelope builder, and `redact(value, home)`. No filesystem, network, or process here; the one write lives in `@attalabs/vinaya`'s `apps/cli/src/lib/log-sink.ts` (`apps/cli/specs/log.md`).
- **The dev-review-loop's policy half** (`src/dev-review-loop/`) — `assessRound(state, observations)` decides every transition of the developer/reviewer/CI loop (dispatch the developer, dispatch reviewers, ask for confidence, publish, or pause with a reason) and returns the `DevReviewLoopEvent`s for the caller to pass to the injected `log()`; `renderSummary(journal)` renders the publication comment (one table, counts by severity per round, confidence, outcome) with no finding text and no line either verdict extractor would read as a real verdict. No filesystem, network, process, or vendor reference anywhere in this directory — the driver (dispatching the developer/reviewers, running CI, calling `log()`) lives elsewhere.

The `bin/` directory carries the check binaries (`verify-dispatch`, `verify-docs`, `verify-coherence`, `open-pr`, `open-issue`, …) that mechanize the AEG gates. They are consumed through the `@attalabs/vinaya` CLI, which bundles this package at build time. `verify-dispatch`'s `Depends-on`/`Conflicts-with` resolver understands three edge shapes — a same-tranche bare task id, a bare `#NNN` Issue reference, and the cross-tranche `<slug> <n>` form — resolving the last one via the same tranche derivation used for the task's own tranche; an edge matching none of these reports `UNRESOLVABLE` rather than being read as an unmerged dependency. `dispatch-gate.ts`'s and `coherence-checks.ts`'s failures each carry a closed, machine-readable class (`DispatchBlockerClass`, `CoherenceFailureCode`) beside the human message string, so a consumer's recovery-prompt switch can be exhaustive over it rather than parsing message text — a new class with no matching prompt fails typecheck instead of silently routing to the wrong advice.
