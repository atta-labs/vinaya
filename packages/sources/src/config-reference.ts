...
export const CONFIG_REFERENCE: readonly ConfigField[] = [
  ...
  },
  {
    ...
    semantics: [
      ...
      'Unlike `ci.setup` — which only ever executes inside a generated, reviewed CI workflow step, under the runner’s own isolation — a declared `tokens.collect` command executes IN-PROCESS, unsandboxed, wherever `vinaya tokens` runs (security review, PR #303, round 2). Two layers close that gap: `vinaya tokens` refuses to run ANY declared command at all until it has been explicitly approved via a one-time `vinaya tokens --trust-collect`, keyed to the exact command string and this repo’s git identity (survives this repo’s own per-task fresh worktrees; a single changed character needs its own fresh approval); once trusted, the exact command is still printed to stderr immediately before every run, as an ongoing audit trail. Approvals live machine-local at `~/.vinaya/tokens-collect-trust.json`, never in any committed file — a pull request can no more grant itself trust than it can add itself to `principals`.',
      'The command itself runs with a bounded timeout — a stuck or hostile command is killed rather than blocking `vinaya tokens` forever — and never falls back to the shipped transcript adapter on any failure (untrusted, timed out, non-zero exit, unparseable output): a declared route that fails is reported as a failure, never silently masked by a different result.'
    ],
    example: `{
  "tokens": {
