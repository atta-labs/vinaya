...
export const COMMANDS: readonly Command[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '`--push` refuses before writing anything when the live body carries no real `AEG:EVIDENCE` pair (it never appends one, unlike `--write`) or when the anchor resolves differently before and after normalisation. After pushing, it re-reads the live body and restores the pre-edit body — exiting non-zero — if anything outside the two anchored regions changed.'
    ],
    status: 'shipped'
  },
  {
