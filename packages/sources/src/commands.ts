...
export const COMMANDS: readonly Command[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Exit and printed summary distinguish a published, reviewed pull request (exit `0`, the PR URL) from a pause (exit `1`, with the exact `vinaya dev-review-loop --resume <pr>` command to continue) — an unattended host tells the two apart from the exit code alone. A run that pauses is resumed with the loop's own existing `--resume <pr>` flag, never a flag on this command."
    ],
    status: 'shipped'
  },
  {
