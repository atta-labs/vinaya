...
export const COMMANDS: readonly Command[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Validates the message file's first line against the same `Type(scope): Description` vocabulary `check`'s `forge-title`-shaped gates enforce on PR/Issue titles — Build, Chore, Docs, Feat, Fix, Perf, Plan, Refactor, Revert, Style, Test. Exits 0 with no output on a conforming message; a `source` of `merge` is skipped outright, since a merge commit's message is written by git, not the person committing."
    ],
    status: 'shipped'
  },
  {
