...
3. **The enforcement layer (referenced by the model, so it must travel with it):**
   - Blast-radius collision domains (conflicts are package-level, `tranche-model.md` §5) — live-derived from `package.json`'s `workspaces` for `packages/*`, plus a built-in cross-cutting default set (lockfile, monorepo config, CI, git hooks); `vinaya.config.json`'s `blastRadius.extraDomains` for anything beyond those. No file to scaffold — the legacy static `.aeg/packages` list still works additively if a repo already has one, but is no longer required or generated.
