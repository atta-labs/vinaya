...
A Milestone is **not** a tranche, and is not required for a tranche to exist. A tranche's identity is its `vinaya/tranche:<slug>` label — never a Milestone. The exception is the legacy case: a Milestone titled exactly a tranche's slug still means that one tranche, forever, for every Milestone created before this layer existed (`tranche-model.md` §4's legacy note). Every Milestone created since is a real many-tranches container, not a slug-matching accident.
...
2. **[named tranches run their own flow]** — each tranche this Milestone names runs the full Tranche flow (`tranche-model.md` §5) independently: Plan → Dispatch → Archive. The Milestone does not orchestrate this directly; it is a passive container whose lifecycle (§4) is computed from what its tranches do.
