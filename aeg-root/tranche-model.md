...
The destination was never the coupled part — no role writes a shared ledger file on a task branch, so layer 3 is already satisfied through the forge everywhere. **An adopter implements layer 2 for their own harness and inherits layers 1 and 3 unchanged.** A doctrine sentence that names a tool belongs in layer 2 or nowhere.
