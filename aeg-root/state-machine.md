...
|---|---|---|
| A1 | `closed-without-merge` | Every closed task-Issue has a merged closing PR, excluding `stateReason: 'not_planned'` closes (`dropped` — a valid terminal state, not a failure) and excluding an Issue hand-closed directly by a recognized Principal with `stateReason: 'completed'` (task `vinaya-engine-v1` 21, #99 — the second, narrower "done" path for a dependency whose technical premise dissolved) |
