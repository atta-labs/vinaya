...
The `SCOPE:` line, and any blast-radius assertion under check 7, are evidence-backed claims: each may be written only after its named check has run — `git diff main --stat` for scope, the consumer grep for multi-project reach — with the output quoted in the same review comment the verdict lands in. The other verdict lines carry no such requirement.
