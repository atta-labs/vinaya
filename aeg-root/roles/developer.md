...
| **Bare digits (whole body)** | `body-bare-digits` (CI) refuses any bare digit outside a fenced/indented/inline code span, an anchored field, or a plain identifier (Issue/PR ref, date, version, path segment, section number) — a countable claim ("138 passed", a duration, a percentage) belongs in a fenced block or doesn't get written. |
