...
These four items govern one decision: whether to author a brief for a task dispatched via `task/<tranche>/<n>` — a task with a Planner's rationale, forge dispatch gates, and a place in a tranche topology. They do not govern a standalone fix: a plain Issue with no such rationale, briefed onto a `fix/<slug>` branch instead, is out of their scope entirely. See **the standalone-fix path**, named after item 4 below, for what does apply there.
...
**The standalone-fix path.** A plain forge Issue with no tranche row, no milestone and no Planner rationale, briefed onto a `fix/<slug>` branch instead of `task/<tranche>/<n>`, is still a brief: `aeg-root/skills/brief-authoring/SKILL.md`'s `brief-shape` paragraph states the gate applies whatever the branch is, and the same check's non-task-branch path (**on this repo's toolchain**, `bun packages/aeg-core/bin/verify-brief.ts --body-file <brief.md>`) validates the same sections without requiring `Closes #N`. Items 1–3 above do not apply to it — there is no dependency graph, no milestone and no eight-field rationale to check. Item 4 applies unchanged: read the surfaces the fix touches and run the same mechanical documentation-update derivation before writing the brief.
