...
Items 1–3 govern one decision: whether to author a brief for a task dispatched via `task/<tranche>/<n>` — a task with a Planner's rationale and forge dispatch gates. They do not govern a standalone fix: a plain Issue with no such rationale, briefed onto a `fix/<slug>` branch instead, has no dependency graph, no milestone and no eight-field rationale for them to check. Item 4's read obligation applies either way. See **the standalone-fix path**, named after item 4 below, for what a standalone fix needs in place of items 1–3.
...
4. **You have read the surfaces.** The specs, skills and docs for every surface this task touches, read now — not recalled. The documentation-update list is derived from that reading plus the mechanical derivation against the current bindings, and the derivation is a procedure, not a judgment call: match the brief's real file surface globs against every `.vinaya/doc-owners` binding's own glob (segment-wise overlap, not exact-string match) and paste the union of fired pointers — or the explicit zero-match line — into the brief before writing the list (the same derivation the Planner runs during Dig; `roles/planner.md` explains it). **On this repo's toolchain**, `bun packages/aeg-core/bin/verify-dispatch.ts <tranche> <n> --surfaces <glob1,glob2,...>` runs this and prints exactly that; where it isn't available, do the glob-match by hand. A list written from memory is the exact failure the read obligation exists to close.

**The standalone-fix path.** A plain forge Issue with no `task/<tranche>/<n>` row, no milestone and no Planner rationale, briefed onto a `fix/<slug>` branch instead, is still a brief: `aeg-root/skills/brief-authoring/SKILL.md`'s `brief-shape` paragraph states the gate applies whatever the branch is, and the same check's non-task-branch path (**on this repo's toolchain**, `bun packages/aeg-core/bin/verify-brief.ts --body-file <brief.md>`) validates the same sections without requiring `Closes #N`. Items 2 and 3 do not apply — there is no dependency graph and no eight-field rationale to check. Item 1 still applies in substance: a standalone fix still needs a real Issue to close (this repo's own `Closes #N` convention doesn't waive that), only its Planner-cuts-it clause doesn't — anyone can file the Issue a standalone fix closes. Item 4's read obligation applies too, but its named command needs a `<tranche> <n>` pair a standalone fix doesn't have: match the brief's file surface against `.vinaya/doc-owners` by hand instead.
