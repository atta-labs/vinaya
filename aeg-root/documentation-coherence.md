...
<!-- AEG:CLAIM: packages/aeg-core/bin/verify-docs.ts contains:const C8_PATHSPECS = -->
<!-- AEG:CLAIM: packages/aeg-core/bin/verify-docs.ts contains:trackedCited.has(path) && existsSync(path) -->
**Claims about code are bound — C8, in `verify-docs`.** A sentence in a doctrine page or a source comment that states what code does, checks, refuses, reads, or returns carries a marker naming the source that proves it, or it is removed. The marker is the premise pin reused — the same `<path> contains:<literal>`, `absent:`, or `sha256:` grammar a brief pins a fact with — on its own line immediately before the paragraph it binds, in either of two forms, and several may stack before one paragraph:
...
What C8 proves is exactly the literal: that the cited file still holds it, still lacks it, or still hashes the same. It cannot prove the sentence is true, so the same division this page draws for C7 holds here — the check verifies the binding, the Reviewer judges the sentence, and an unbound behavioural claim is theirs to call. That division is what makes the literal a choice worth making deliberately: pick one whose change would falsify the sentence, which for a claim about a read window means the function that windows the read, never another comment. Two comments agreeing proves only that both were written on the same day. Its corpus is every doctrine page under `aeg-root/` and every tracked `.ts` or `.md` file under `apps/` and `packages/`: full mode sweeps all of them, and PR mode blocks on the ones the diff actually touched. A cited file is read only when git tracks it, so a binding always rests on something a reviewer can open. A marker inside a fenced block — this page's own example above — is documentation, not a claim, and is never evaluated. The marker itself is an HTML comment of the same shape as an `AEG:QUOTES-FILE` marker: whatever a rendering site does with one it does with the other, and this rule asks nothing new of any renderer.
