...
- The brief must be in the PR body, unmodified — pasted verbatim, not summarized or paraphrased. The whole body is authored once, at open, and stays unmodified after open except the `AEG:EVIDENCE` block and one appended `AEG:TOKENS` row for a re-entry turn, both written through `pr edit`, never by hand. A review round's response and any re-run evidence are PR comments, not body edits.
