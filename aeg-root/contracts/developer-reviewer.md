...
| **Every copy of a false doc claim fixed** (the doc-correctness finding's `Search:` pattern re-run with no path filter, output pasted in the round response; the corrected sentence bound with `AEG:CLAIM` where it states what code does) | Re-review of that finding id | The Reviewer re-runs the same `Search:` pattern at the new head; the id is resolved only when every remaining hit is a true statement. A missing pasted search output, a still-false hit, or an unbound corrected claim keeps the id open — a BLOCKER id keeps driving the verdict. |
