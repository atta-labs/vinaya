...
- `pr report` derives a `Summary:` line (files, insertions, deletions, binary
  ...
  resolve the anchor pair and the summary line from one body-wide mask, so the
  line that is exempt is the line that is compared. `evidence-fresh` also
  refuses a region hidden inside a `<details>` block or a fence, where nothing
  could verify it, rather than skipping it as unadopted.
